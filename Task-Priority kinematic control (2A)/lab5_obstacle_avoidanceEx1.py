...
custom_colors = ['red', 'green', '#C79FEF', 'blue']

if len(tasks) == 1 and tasks[0].name == "Configuration task": 
    ax_q.plot(time_log, err_log[:, 0], label='e1 End-effector position', color='blue')
    ax_q.plot(time_log, err_log[:, 1], label='e2 End-effector orientation', color='cyan')
else:
    for i in range(len(tasks)):
        err_string = f"d_{i+1}" if "obstacle" in labels[i] else f"e_1" if "position" in labels[i] else f"e_2" if "orientation" in labels[i] else f"q_{i+1}"
        
        # Assign colors based on task type
        current_color = custom_colors[i % len(custom_colors)]
        
        ax_q.plot(time_log, err_log[:, i], 
                  label=f'{err_string} ({labels[i]})', 
                  color=current_color)

ax_q.set_title('Task-Priority Error Tracking')
ax_q.set_xlabel('Time[s]')
ax_q.set_ylabel('Error[m/rad]')
ax_q.legend()
ax_q.grid(True)
