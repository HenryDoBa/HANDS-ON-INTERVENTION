...
if len(tasks) == 1 and tasks[0].name == "Configuration task": # Configuration task requires special ploting
    ...
    for i in range(len(tasks)):
        ...
        if "joint limit" in labels[i]:
            # Plot joint limits as horizontal dashed lines
            ax_q.axhline(y=joint_limit[0], color='red', linestyle='--', label='Lower Limit')
            ax_q.axhline(y=joint_limit[1], color='red', linestyle='--', label='Upper Limit')
